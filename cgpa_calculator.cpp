...
public:
    ...
    static bool caseInsensitiveCompare(const string &lhs, const string &rhs) {
        ...
        for (size_t i = 0; i < lhs.size(); ++i) {
            if (static_cast<char>(::tolower(static_cast<unsigned char>(lhs[i]))) !=
                static_cast<char>(::tolower(static_cast<unsigned char>(rhs[i])))) {
                return false;
            }
        }
        return true;
...
private:
    ...
    void recordRejectedWord(const string &value) {
        if (value.empty()) {
            return;
        }
        rejectedWords.push_back(value);
        updateMemoryUsage();
    }
    ...
    void updateMemoryUsage() {
        ...
        total += customScores.size() * (sizeof(int) * 2);
        total += rejectedWords.size() * sizeof(string);
        for (const auto &rejected : rejectedWords) {
            total += rejected.size();
        }
        total += revealedPositions.size() * sizeof(size_t);
        total += currentWord.size();
